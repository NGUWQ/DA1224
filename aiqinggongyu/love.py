...
def getinfo():
    love = pd.DataFrame(columns=['date','score','city','comment','nick'])
    for i in range(0,1000):
        j=random.randint(1,1000)
        print(str(i) + ' ' + str(j))
        try:
            time.sleep(2)
            url='http://m.maoyan.com/mmdb/comments/movie/1175253.json?_v_=yes&offset='+str(j)
            html=requests.get(url=url,headers=headers).content
            data=json.loads(html.decode('utf-8'))['cmts']
            for item in data:
                love=love.append({'date':item['time'].split(' ')[0],'city':item['cityName'],'score':item['score'],
                                  'comment':item['content'],'nick': item['nick']},ignore_index=True)
            love.to_excel('爱情公寓2.xlsx',index=False)
...
#去重
def quchong():
    data=pd.read_excel('C:\爬虫\爱情公寓\爱情公寓3.xlsx')
    data=data.drop_duplicates()
    data=pd.DataFrame(data)
    data.to_excel('C:\爬虫\爱情公寓\爱情公寓.xlsx')


#分析图表
def charts():
    #可以直接读取我们已经爬到的数据进行分析
    love_com=pd.read_excel('C:\爬虫\爱情公寓\爱情公寓.xlsx')
    grouped=love_com.groupby(['city'])
    ...
    v1 = city_main['count']
    v2 = city_main['mean']
    line = Line("《爱情公寓》主要城市评分")
    line.add("分数", attr, v2, xaxis_rotate=30,
             mark_point=['min', 'max'], xaxis_interval=0, line_color='lightblue',
             line_width=4, mark_point_textcolor='red', mark_point_color='#ccf',
             is_splitline_show=False, is_stack=True)
    bar = Bar("《爱情公寓》主要城市评论数")
    bar.add("城市", attr, v1, xaxis_rotate=30, yaxis_min=0, xaxis_interval=0, is_splitline_show=False, is_stack=True)
    overlap = Overlap()
    # 默认不新增x y 轴,并且x y轴的索引都为0
    overlap.add(bar)
    overlap.add(line, yaxis_index=1, is_add_yaxis=True)
    overlap.render('爱情公寓主要城市评论数_平均分.html')
    ...
    geo = Geo('《爱情公寓》全国主要城市打分图', title_color="#fff",
              title_pos="center", width=1200, height=600, background_color='#404a59')
    attr, value = geo.cast(data)
    geo.add("", attr, value, visual_text_color="#fff", symbol_size=15, is_visualmap=True,
            is_roam=False, visual_range=[0, 200])
    geo.render('爱情公寓全国主要城市打分图.html')

    # 绘制词云
    love_str = ''.join(love_com['comment'])
    words_list = []
    word_generator = jieba.cut_for_search(love_str)
    for word in word_generator:
        words_list.append(word)
    words_list = [k for k in words_list if len(k) > 1]

    back_color = imread('C:/爬虫/爱情公寓/love.jpg')  # 解析该图片
    wc = WordCloud(
    ...
    love_count = Counter(words_list)
    wc.generate_from_frequencies(love_count)
    # 基于彩色图像生成相应彩色

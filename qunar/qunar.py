...
def savetocsv():
    with open('travel.csv', 'a+', encoding='gbk') as csvfile:
        fieldnames = ['时间', '出发地', '目的地', '价格', '天数', '亮点', '出行工具', '类别']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

def savetocsv2(result):
    with open('travel.csv', 'a+', encoding='gbk',newline='') as csvfile:
        fieldnames = ['时间', '出发地', '目的地', '价格', '天数', '亮点', '出行工具', '类别']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
